...
function getIntSemanticFirmwareVersion(version) {
  (getIntFirmwareMajorVersion(version) << 16) |
    (getIntFirmwareMinorVersion(version) << 8) |
    getIntFirmwarePatchVersion(version);
}
...
function getIntFirmwareVersion(version) {
  return (
    (getArchitecture(version) << 24) | getIntSemanticFirmwareVersion(version)
  );
}
